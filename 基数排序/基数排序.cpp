...
unsigned int getRadixValue(unsigned int value, unsigned int radix, unsigned int radixCount) {//值、位、计算位数 
...
bool radixSort(unsigned int array[], unsigned int arrLen, unsigned int radix) {//数组、数组长度 
    if (arrLen <= 1) {
        return true;//判断数组需不需要排序 
    }
    ...
    int radixCount = 0;
    while ((max/=radix) > 0) {//计算基数为多少位，有多少位要进行排序 
        ++radixCount;
    }

    // Create counting buckets
    int *countingRadixBuckets = new int[radix]();//计数器 ，分桶：利用基数来创建桶 
    int *tempArray = new int[arrLen]();//创建临时数组记录排序后的数组 
    ...
    for (size_t radixIndex = 0; radixIndex <= radixCount; ++radixIndex)
    {
        ...
        {
            ++countingRadixBuckets[getRadixValue(array[j], radix, radixIndex)];//找到所在位的值再对桶统计 
        }

        // Accumulate coutingBuckets to find the last ordered location of value in array
        for (size_t k = 1; k < radix; ++k)
        {
            countingRadixBuckets[k] += countingRadixBuckets[k-1];//按位排序 
        }
        ...
        for (size_t m = 0; m < arrLen; ++m)//将临时数组的内容复制到数组array中 
        {
            array[m] = tempArray[m];//此时有序了 
        }

        for (size_t n = 0; n < radix; ++n)
        {
            countingRadixBuckets[n] = 0;//清零，返回 
        }
